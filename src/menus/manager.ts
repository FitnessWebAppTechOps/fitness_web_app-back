...
export class MenusManager {
  ...
  static async getMenuById(menuId: string): Promise<IMenuDocument> {
    return MenusModel.findById(menuId)
      .orFail(new DocumentNotFoundError(menuId))
      .exec();
  }

  static async getMenuByUserId(userId: string): Promise<IMenuDocument> {
    return MenusModel.findById(userId)
      .orFail(new DocumentNotFoundError(userId))
      .exec();
  }

  static async updateMenuById(
    menuId: string,
    update: Partial<IMenu>
  ): Promise<IMenuDocument> {
    return MenusModel.findByIdAndUpdate(menuId, update, { new: true })
      .orFail(new DocumentNotFoundError(menuId))
      .exec();
  }

  static async deleteMenu(menuId: string): Promise<IMenuDocument> {
    return MenusModel.findByIdAndDelete(menuId)
      .orFail(new DocumentNotFoundError(menuId))
      .exec();
  }
}
